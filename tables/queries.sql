...
select AMOUNT from PENALTIES where AMOUNT >= 50 and AMOUNT <= 100;


/*6. output of PlayerId, name of players who do not live in Stratford or Douglas.*/
select PLAYERNO, NAME from PLAYERS where TOWN not in ('Stratford', 'Douglas');
...
select EMPNO, ENAME, DEPTNO from EMP where DEPTNO = 30 and SAL >= 1500;

/* 11. output of all managers who do not belong to department 30. */
select MGR from EMP where DEPTNO not in (30);


/* 12. output of all employees from department 10 who are neither managers nor clerical workers (CLERK). */
select EMPNO, ENAME from EMP where DEPTNO = 10 and JOB not in ('Manager', 'Clerk');


/* 13. output of all employees who earn between 1200,- and 1300,-. */
select EMPNO, ENAME from EMP where SAL > 1200 and SAL < 1300;
...
select * from EMP where HIREDATE > TO_DATE('4-JAN-1981','DD-MON-YYYY') and HIREDATE < TO_DATE('15-APR-1981','DD-MON-YYYY');
